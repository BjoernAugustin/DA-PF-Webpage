...
export class SlidesService {
  ...
  nextSlide() {
    this.activeSlideIndex = (this.activeSlideIndex + 1) % this.content.length;
  }

  previousSlide() {
    this.activeSlideIndex = (this.activeSlideIndex - 1 + this.content.length) % this.content.length;
  }

  getSlideClass(index: number) {
    if (index === this.activeSlideIndex) {
      return 'active';
    } else if (index === (this.activeSlideIndex - 1 + this.content.length) % this.content.length) {
      return 'slide-previous';
    } else if (index === (this.activeSlideIndex + 1) % this.content.length) {
      return 'slide-next';
    }
    return 'hidden';
  }
